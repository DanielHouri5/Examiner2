    ...
    for i in range(n):
        for j in range(n):
            if i < j:
                U[i][j] = mat[i][j]
    return U
def get_gauss_seidel_H(mat, n):
    return np.linalg.inv(get_L(mat, n) + get_D(mat, n))


def get_gauss_seidel_G(mat, n):
    return np.dot(np.linalg.inv(get_L(mat, n) + get_D(mat, n)), get_U(mat, n))*(-1)


#########################################################


def gauss_seidel(mat, b, n, X0, TOL=0.001):
    H = get_gauss_seidel_H(mat, n)
    G = get_gauss_seidel_G(mat, n)
    k = 1
    print("Gauss seidel:")
    print("Iteration" + "\t\t\t".join([" {:>12}".format(var) for var in ["x{}".format(i) for i in range(1, len(mat) + 1)]]))
    print("-----------------------------------------------------------------------------------------------")
    while True:
...
def get_gauss_seidel_solution(mat, b, n, X0):
    if check_if_singular(mat, b, n) == -1:
        if not is_diagonally_dominant(mat):
            print('Matrix is not diagonally dominant!\n')
        else:
            return gauss_seidel(mat, b, n, X0)
...
if __name__ == '__main__':
    ...
    solution = get_gauss_seidel_solution(mat, b, n, x)
    print(solution, "\n")
